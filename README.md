...
- **Конфиг:** `notebooks/make_dataset_scripts/config.yaml` задаёт Spark-параметры, окна train/test, пути до внешних эмбеддингов (`paths.embeddings`) и контентных слов (`paths.context_words`), а также схему сохранения `outputs.save_schema`. В финальной версии модели из источников используются:
...
!hdfs dfs - -mkdir hdfs://arnsdpsbx/user/22685380_omega-sbrf-ru/data/data_24
!hdfs dfs -cp viewfs://SDP-leverkin-ap-ca-sbrf-ru-SberSovetnik-471b9a/user/22685380_omega-sbrf-ru/deepfm_data/data_24/* hdfs://arnsdpsbx/user/22685380_omega-sbrf-ru/data/data_24
```
...
- В конструктор передаются пути из dataset-конфига: директория parquet (`parquet_dir`), словари признаков/эмбеддингов, маппинги `inn_* -> *_index` и списки уникальных `dt/kt`.
...
3. **Запуск.**
   ...
   Одна эпоха на двухмесячном датасете занимает порядка 4-5 часов обучения + столько же на валидацию после эпохи. Имейте в виду, что после примерно 24 часов бездействия JupyterHub автоматически вырубается, даже если там крутится обучение, поэтому нужно регулярно заходить, чтобы убедиться, не сдохла ли модель. Можно поставить `save_period` на 1, чтобы сохранялся результат каждой эпохи, но модели довольно много весят и есть риск забарахлить директорию.

   Модели, полученные у меня, содержат порядка 500М (500 миллионов) параметров и весят в районе 6 ГБ. Две модели (от kt к dt и от dt к kt) суммарно, таким образом, весят 12 ГБ.
...
2. `python3 train.py --config-name test_reverse ...` с переставленными `*_feature_sizes`.
