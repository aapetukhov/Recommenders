...
## Где взять артефакты?
Артефакты модели (датасеты, код, логи, лучшие версии обученных моделей) лежат в командном хранилище по адресу
`hdfs://arnsdpsbx/user/team/team_fin_adviser/petukhov_andrey_deepfm_code_and_artifacts`

Скопировать их себе в среду разработки в jupyterhub (среда с GPU-машинками в омеге, находящаяся по адресу `jupyterhub-datalab.apps.prom-datalab.ca.sbrf.ru`) можно командой
```bash
hdfs dfs -get hdfs://arnsdpsbx/user/team/team_fin_adviser/petukhov_andrey_deepfm_code_and_artifacts .
```
