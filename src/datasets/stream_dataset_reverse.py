...
class StreamDatasetReverse(IterableDataset):
    """
    Streaming dataset for kt->dt direction.

    User tower now consumes kt features and external embeddings, while
    item tower works with dt descriptors. The rest of the streaming
    behaviour (positive + random negative sampling) mirrors the original
    ...
    def __init__(
        ...
        self.dt_topic_default = (
            np.zeros(self.dt_topic_dim, dtype=np.float32) if self.dt_topic_dim else None
        )
        self.kt_topic_default = (
            np.zeros(self.kt_topic_dim, dtype=np.float32) if self.kt_topic_dim else None
        )
    ...
    def parse_data(self, chunk):
        for _, row in chunk.iterrows():
            ...
            inn_dt_neg = random.choice(self.unique_dt)
            while inn_dt_neg == inn_dt:
                inn_dt_neg = random.choice(self.unique_dt)
            yield self.make_sample(inn_dt_neg, inn_kt, label=0)
    ...
    def make_sample(self, inn_dt, inn_kt, label):
        ...
        kt_data = self.kt_feat.get(kt_idx, {})

        sample = {
            "label": torch.tensor(label, dtype=torch.float32),
            # note: user tower now consumes kt descriptors
            "user": torch.tensor(
            ...
            "kt_emb": torch.tensor(
                self.kt_emb.get(kt_idx, np.zeros(256)), dtype=torch.float32
            ),
            "dt_emb": torch.tensor(
                self.dt_emb.get(dt_idx, np.zeros(256)), dtype=torch.float32
            ),
        }
