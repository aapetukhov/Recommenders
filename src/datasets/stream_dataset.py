...
class StreamDataset(IterableDataset):
    ...
    def make_bpr_sample(self, inn_dt, inn_kt_pos, inn_kt_neg):
        ...
        return {
            ...
            # positive item info
            "kt_emb_pos": torch.tensor(
                self.kt_emb.get(self.inn_kt_to_index[inn_kt_pos], np.zeros(256)),
                dtype=torch.float32,
            ),
            "item_pos": torch.tensor(
                [kt_data_pos.get(f, 0) for f in self.kt_features], dtype=torch.long
            ),
            "double_item_pos": torch.tensor(
                [kt_data_pos.get(f, 0.0) for f in self.kt_double_features],
                dtype=torch.float32,
            ),
            # negative sample info
            "kt_emb_neg": torch.tensor(
                self.kt_emb.get(self.inn_kt_to_index[inn_kt_neg], np.zeros(256)),
                dtype=torch.float32,
            ),
            "item_neg": torch.tensor(
                [kt_data_neg.get(f, 0) for f in self.kt_features], dtype=torch.long
            ),
            "double_item_neg": torch.tensor(
                [kt_data_neg.get(f, 0.0) for f in self.kt_double_features],
                dtype=torch.float32,
            ),
        }
