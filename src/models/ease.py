...
class SparseEASE:
    ...
    def fit(self, X: csr_matrix):
        ...
        G = G + diags([self.lambda_], [0], shape=(G.shape[0], G.shape[0]))

        P: csr_matrix = inv(G)

        diag_P = P.diagonal()
        B: csr_matrix = P.multiply(1 / -diag_P[:, None])
        B = B.tolil()
        B[diag_indices, diag_indices] = 0

        self.B = B.tocsr()
        self.preds = X.dot(self.B)
    ...
    def predict_score(self, user_inn: int, item_inn: int):
        ...
        return self.X[user_id, :].dot(self.B[:, item_id])[0, 0]

    def predict_for_kt(self, user_inn: int, interactions_set: Set[int]) -> pd.DataFrame:
        """Prediction for a single user"""
        user_id = self.inn2id[user_inn]
        scores = self.X[user_id, :].dot(self.B).toarray().flatten()
