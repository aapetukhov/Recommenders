...
logging.basicConfig(
    level=logging.INFO,
    filename=f"{data_dir}/{config['outputs']['log_file']}",
    format='%(asctime)s %(levelname)s %(message)s'
)
logger = logging.getLogger(__name__)
logger.info(f"Папка для словарей: {data_dir}")
logger.info("Запускаем Spark")

conf = SparkConf() \
    .setAppName(config["spark"]["appName"]) \
    .setMaster(config["spark"]["master"]) \
    .set("spark.executor.instances", config["spark"]["executor_instances"]) \
    .set("spark.executor.cores", config["spark"]["executor_cores"]) \
    .set("spark.driver.memory", config["spark"]["driver_memory"]) \
    .set("spark.executor.memory", config["spark"]["executor_memory"]) \
    .set("spark.executor.memoryOverhead", config["spark"]["executor_memoryOverhead"]) \
...
# PARAMETERS
train_date_start = config["dates"]["train_date_start"]
train_date_end = config["dates"]["train_date_end"]
test_date_start = config["dates"]["test_date_start"]
test_date_end = config["dates"]["test_date_end"]
save_schema = config["outputs"]["save_schema"]
logger.info("Параметры дат установлены")
...
with open(f"{data_dir}/kt_mappings.pkl", "wb") as f:
    pickle.dump(kt_mappings, f)
with open(f"{data_dir}/dt_mappings.pkl", "wb") as f:
    pickle.dump(dt_mappings, f)
with open(f"{data_dir}/kt_num_embeddings.pkl", "wb") as f:
    pickle.dump(kt_num_embeddings, f)
with open(f"{data_dir}/dt_num_embeddings.pkl", "wb") as f:
...
# TODO: CHANGE!!! IT DOES NOT WORK
test_dict = {row["inn_dt_index"]: row["kt_set"] for row in test_dict_df.collect()}

with open(f"{data_dir}/test_dict.pkl", "wb") as f:
...
train_name = config["outputs"]["train_name"]
interactions_train_indexed.repartition(1000)\
    .write.mode("overwrite").saveAsTable(f"{save_schema}.{train_name}")
logger.info(f"Витрина {train_name} сохранена в {save_schema}")

test_name = config["outputs"]["test_name"]
test_df.repartition(1000)\
    .write.mode("overwrite").saveAsTable(f"{save_schema}.{test_name}")
logger.info(f"Витрина {test_name} сохранена в {save_schema}")
...
with open(f"{data_dir}/dt_features_dict.pkl", "wb") as f:
    pickle.dump(dt_features_dict, f)
logger.info(f"Словарь dt_features_dict сохранен в {data_dir}/dt_features_dict.pkl")
...
with open(f"{data_dir}/kt_features_dict.pkl", "wb") as f:
    pickle.dump(kt_features_dict, f)
logger.info(f"Словарь kt_features_dict сохранен в {data_dir}/kt_features_dict.pkl")
...
with open(f"{data_dir}/unique_inn_dt_indices.pkl", "wb") as f:
    pickle.dump(dt_unique, f)
logger.info(f"Словарь unique_inn_dt_indices сохранен в {data_dir}/unique_inn_dt_indices.pkl")
with open(f"{data_dir}/unique_inn_kt_indices.pkl", "wb") as f:
    pickle.dump(kt_unique, f)
logger.info(f"Словарь unique_inn_kt_indices сохранен в {data_dir}/unique_inn_kt_indices.pkl")
...
with open(f"{data_dir}/dt_embeddings_dict.pkl", "wb") as f:
    pickle.dump(dt_emb_dict, f)
logger.info(f"Словарь dt_embeddings_dict сохранен в {data_dir}/dt_embeddings_dict.pkl")
...
with open(f"{data_dir}/kt_embeddings_dict.pkl", "wb") as f:
    pickle.dump(kt_emb_dict, f)
logger.info(f"Словарь kt_embeddings_dict сохранен в {data_dir}/kt_embeddings_dict.pkl")
